...
// the WPILib BSD license file at: https://github.com/wpilibsuite/allwpilib/blob/main/LICENSE.md

// Copyright (c) NullFTC & WPILib Contributors.

package dev.nullftc.wpiftc.localization;
 ...
 * FTCPoseEstimator#addVisionMeasurement} and only call {@link FTCPoseEstimator#update} then this will
 * behave exactly the same as Odometry.
 *
 * <p>{@link FTCPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link FTCPoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you
 ...
 * @since 06.13.2025
 */
public class FTCPoseEstimator<T> {
    ...
    public Optional<Pose2d> sampleAt(double timestampSeconds) {
        ...
        return odometryEstimate.map(odometryPose -> {
            assert visionUpdate != null;
     ...
     * FTCPoseEstimator#update} every loop.
     *
     ...
     *     FTCPoseEstimator#updateWithTime(double,Rotation2d,Object)} then you must use a timestamp with
     *     an epoch since FPGA startup (i.e., the epoch of this timestamp is the same epoch as .) This means that you should use  as your time source or sync the epochs.
     */
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        ...
        Optional<Pose2d> sample = m_odometryPoseBuffer.getSample(timestampSeconds);
...
//        var k_times_twist = m_visionK.times(VecBuilder.fill(twist.dx, twist.dy, twist.dtheta));
        ...
        VisionUpdate visionUpdate = new VisionUpdate(visionSample.get().exp(scaledTwist), sample.get());
     ...
     * FTCPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
     *
     * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
     * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
     ...
     *     should use Timer#getFPGATimestamp as your time source in
     *     this case.
     * @param visionMeasurementStdDevs Standard deviations of the vision pose measurement (x position
     *     in meters, y position in meters, and heading in radians). Increase these numbers to trust
     *     the vision pose measurement less.
    ...
    public Pose2d update(Rotation2d gyroAngle, T wheelPositions) {
        return updateWithTime(System.nanoTime() / 1_000_000_000.0, gyroAngle, wheelPositions);
    }
    ...
    private static final class VisionUpdate {
        ...
        public Pose2d compensate(Pose2d pose) {
            Transform2d minus = pose.minus(odometryPose);
            return this.visionPose.plus(minus);
        }
    }
}